...
const ActivityBookingsManager = () => {
  ...
  // Fetch bookings and activity details
  useEffect(() => {
    if (businessId) {
      fetchBookings();
  ...
  const fetchBookings = async () => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/reservation/business/${businessId}/Activity`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem(
              'token_partner_activities'
            )}`
          }
        }
      );
  ...
  const updateRegistrationStatus = async () => {
    try {
      ...
      toast.success(
        `New bookings are now ${newStatus ? 'allowed' : 'blocked'}`
      );
    } catch (error) {
    ...
    <div className="bg-white mt-2 rounded-lg shadow w-full px-6">
      <div className="p-4 border-b">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-4">
            ...
            >
              Past Bookings ({inactiveBookings.length})
            </button>
          </div>
      ...
      (activeTab === 'inactive' && inactiveBookings.length === 0) ? (
        <div className="text-center py-8 text-gray-500">
          {selectedDate
            ? `No ${activeTab} bookings found for ${new Date(
                selectedDate
              ).toLocaleDateString()}`
            : `No ${activeTab} bookings found matching your search.`}
        </div>
      ) : null}
    </div>
  );
